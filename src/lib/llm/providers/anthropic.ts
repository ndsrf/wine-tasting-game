  ...
  "wines": [
    {
      ...
      const message = await this.client.messages.create({
        ...
        max_tokens: 2000,
        temperature: 0.7,
        system: 'You are a professional sommelier with deep knowledge of wine characteristics. Respond in English only. Always respond with valid JSON only.',
        messages: [
          {
            role: 'user',
            content: prompt,
...
Keep each explanation concise (4-5 sentences) but informative.`;
      ...
      const message = await this.client.messages.create({
        ...
        max_tokens: 2000,
        temperature: 0.7,
        system: `You are a professional sommelier providing educational explanations about wine characteristics. Respond in ${targetLanguage}. Always respond with valid JSON only.`,
        messages: [
          {
            role: 'user',
            content: prompt,
      ...
      const explanations = JSON.parse(content.text);
      return explanations;
    } catch (error) {
      console.error('Anthropic API error for explanations:', error);
...
Example bad hint: "Château Margaux typically shows..." (mentions wine name)`;
      ...
      const message = await this.client.messages.create({
        ...
        max_tokens: 150,
        temperature: 0.7,
        system: `You are a helpful sommelier providing educational hints without giving away answers. Respond in ${targetLanguage}.`,
        messages: [
          {
            role: 'user',
            content: prompt,
      ...
      console.error('Anthropic API error for hint generation:', error);
