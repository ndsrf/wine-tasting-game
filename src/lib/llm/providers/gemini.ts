  ...
  "wines": [
    {
      ...
      const lowConfidenceWines = parsedResult.wines.filter((wineData: any) => {
      ...
      const processedWines = parsedResult.wines.map((wineData: any, index: number) => ({
        wine: wines[index],
...
Keep each explanation concise (4-5 sentences) but informative.`;
      ...
      console.error('Gemini API error for explanations:', error);
...
Example bad hint: "Château Margaux typically shows..." (mentions wine name)`;
      ...
      console.error('Gemini API error for hint generation:', error);
